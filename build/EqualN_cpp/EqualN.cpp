...
void EqualN_2_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void EqualN_2_run(uint ctx_index,Circom_CalcWit* ctx);
...
ctx->componentMemory[coffset].templateName = "EqualN";
ctx->componentMemory[coffset].signalStart = soffset;
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "IsEqual_17_381";
...
for (uint i = 0; i < 3; i++) {
std::string new_cmp_name = "IsEqual_13_281"+ctx->generate_position_array(aux_dimensions, 1, i);
IsEqual_1_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
mySubcomponents[aux_create+i] = aux_cmp_num;
csoffset += 6 ;
aux_cmp_num += 2;
...
Fr_lt(&expaux[0],&lvar[3],&circuitConstants[3]); // line circom 12
...
PFrElement aux_dest = &lvar[2];
// load src
Fr_add(&expaux[0],&lvar[2],&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[3])) + 6)]); // line circom 14
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
PFrElement aux_dest = &lvar[3];
// load src
Fr_add(&expaux[0],&lvar[3],&circuitConstants[1]); // line circom 12
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
PFrElement aux_dest = &lvar[1];
// load src
Fr_add(&expaux[0],&lvar[1],&circuitConstants[1]); // line circom 12
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[3],&circuitConstants[3]); // line circom 12
...
EqualN_2_create(1,0,ctx,"main",0);
EqualN_2_run(0,ctx);
}
