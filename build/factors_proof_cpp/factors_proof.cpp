...
void factors_proof_3_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void factors_proof_3_run(uint ctx_index,Circom_CalcWit* ctx);
...
ctx->componentMemory[coffset].templateName = "factors_proof";
ctx->componentMemory[coffset].signalStart = soffset;
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "multiply_22_409";
multiply_0_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
mySubcomponents[aux_create+i] = aux_cmp_num;
csoffset += 3 ;
aux_cmp_num += 1;
}
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "IsEqual_23_442";
...
ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1;
assert(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter));
multiply_0_run(mySubcomponents[cmp_index_ref],ctx);
}
{
...
factors_proof_3_create(1,0,ctx,"main",0);
factors_proof_3_run(0,ctx);
}
